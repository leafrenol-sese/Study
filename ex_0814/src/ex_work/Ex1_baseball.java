...
public class Ex1_baseball {
    public static void main(String[] args) {
        ...
        Outer: for (int i = 0; i < com.length;) {
            ...
            i++;// 컴퓨터가 만든 숫자에 중복이 없다면 i값 증가

        } // outer

        System.out.println("정답 : " + com[0] + com[1] + com[2]); //컴퓨터가 만든 결과값(본방 게임이라면 안보이게 하고 게임시작)
        ...
        int cnt = 0; // 몇번만에 맞췄는가 while밖에서 세어야 초기화되지 않는다
        ...
        while (true) {
            ...
            if (strike == 3) {
                ...
                if (strike > 0 || ball > 0) { // strike 또는 ball의 수가 0보다 클 때
                    ...
                    System.out.println("OUT!"); //인덱스도, 데이터도 겹치는것이 없을 때
                }
            }
